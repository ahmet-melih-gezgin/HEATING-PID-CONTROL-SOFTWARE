...
setpoint=110;
  ...
  setup_timer_2(T2_DIV_BY_16,255,1);   // PWM period = (PR2+1) x 4 x Tosc x TMR2 prescaler///prescale=16, PR2=195, postscale=16  100HZ ÝÇÝN
  set_pwm1_duty(0);                   
  set_tris_c(0x00); 
  set_tris_b(0xFF);
  setup_CCP1(CCP_PWM);                  
...
ykt=voltaj;
rkt=setpoint;
ekt=rkt-ykt;//Hata hesaplanýyor
pkt=b*ekt+pkt_l;
qkt=c*(ekt-ekt_l);
ukt=pkt+a*ekt+qkt;// PID Hesaplanýyor
...
ukt=ukt-3.0;
ukt=ukt/4.0;
control=(int16)ukt;//PWM Sinyali int Dönüþtürülüyor.Float tipinde olamaz.
set_pwm1_duty(control);//PWM Sinyali Gönderiliyor
pkt_l =pkt;
ekt_l=ekt;
delay_ms(200);
 /* Setpoint'i arttýrma azaltma butonlarý*/
 if(input(pin_b5)==0)  setpoint arttýr
 {
    delay_ms(10);
    setpoint=setpoint+1;
 ...
 itoa(control,5,str3);//Seri Haberleþme Terminaline gönderilen sinyal ASCII tipinden çýkarýldý.
 puts("\n\r");
 putc(str3[0]);
 putc(str3[1]);/*putc((int8)control);//control = control>8;//putc((int8)control); Bu türde yazýlabilir.*/
    }
